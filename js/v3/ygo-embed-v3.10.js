(() => {
  ...
  function convertDeckList(p, section, cardList) {
    let cards;
    try {
        // Always treat as JSON since the input is already in JSON format
        cards = JSON.parse(cardList);
        console.log('Parsed deck list:', cards);

        if (!Array.isArray(cards) || cards.length === 0) {
            throw new Error("Invalid deck list format - must be a JSON array");
        }
        ...
        console.error("Error parsing deck list:", err);
        console.error("Raw card list:", cardList);
        p.innerHTML = `<div class="ygo-error">❌ Error parsing deck list: ${err.message}</div>`;
    }
  }
