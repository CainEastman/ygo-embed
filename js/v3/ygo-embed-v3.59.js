(() => {
  ...
  function generateCardStats(card) {
    if (card.type.toLowerCase().includes('monster')) {
      ...
      // For Spell/Trap cards, show their specific type
      const cardType = card.type.toLowerCase();
      let fullType = card.type; // Default to just "Spell Card" or "Trap Card"
      
      // Add specific spell/trap type if available
      if (card.race) {
        if (cardType.includes('spell')) {
          fullType = `${card.race} Spell`;
        } else if (cardType.includes('trap')) {
          fullType = `${card.race} Trap`;
        }
      }
      
      return `<div class="ygo-card-stats">
                <span class="ygo-card-type">${fullType}</span>
              </div>`;
    }
  }
