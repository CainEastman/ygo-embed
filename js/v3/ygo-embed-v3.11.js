(() => {
  ...
  function convertDeckList(p, section, cardList) {
    let cards;
    try {
        console.log('Converting deck list:', {
            section,
            rawInput: cardList,
            inputType: typeof cardList,
            length: cardList.length
        });

        // Remove any surrounding quotes and clean whitespace
        let cleanCardList = cardList.trim();
        
        // Handle potential double-encoded JSON
        try {
            // If it's a string that looks like JSON but is encoded as a string
            if (cleanCardList.startsWith('"[') && cleanCardList.endsWith(']"')) {
                cleanCardList = JSON.parse(cleanCardList);
            }
        } catch (e) {
            console.log('Not double-encoded JSON:', e);
        }

        // Remove any remaining surrounding quotes
        cleanCardList = cleanCardList.replace(/^["'](.*)["']$/s, '$1').trim();
        
        console.log('Cleaned card list:', cleanCardList);

        // First try parsing as JSON
        try {
            cards = JSON.parse(cleanCardList);
            console.log('Successfully parsed as JSON:', cards);
        } catch (e) {
            console.log('JSON parsing failed:', e);
            // If JSON parsing fails, split by commas while preserving quoted strings
            cards = cleanCardList
                .match(/(?:[^\,"]|"(?:[^"])*")+/g)
                .map(entry => entry.trim())
                .filter(entry => entry.length > 0)
                .map(entry => entry.replace(/^["'](.*)["']$/, '$1').trim());
            console.log('Parsed as comma-separated list:', cards);
        }

        if (!Array.isArray(cards) || cards.length === 0) {
            throw new Error("Invalid deck list format - must be an array of card names");
        }
        ...
        console.log(`Successfully converted deck list for section "${section}":`, {
            raw: cardList,
            cleaned: cleanCardList,
            parsed: cards
        });
    } catch (err) {
        console.error("Error parsing deck list:", {
            error: err,
            raw: cardList,
            section: section
        });
